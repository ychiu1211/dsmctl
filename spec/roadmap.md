...
| [WI-090](work-items/WI-090-snapshot-replication-mutation.md) | P2 | `in_progress` | C | WI-089 | Guarded Snapshot Replication **relation create** source→dest (temp_create → check_remote_conn → DR.Plan create v3 → poll) via hash-bound plan/apply; dest credential resolved from its vault profile **at apply only** (never plan/hash/logs/MCP/model — live-verified no-leak). Never-overwrite + health + reachability guards, both-revision binding, async poll-then-confirm; adversarially reviewed (5 fixes). Failover family read-only `can_*` only. **BLOCKED at DSM's `synocredential` credential-mint**: the apply ran unblocked to `temp_create`, which needs an OAuth-broker handle, not a raw session (err 528). Vault-brokering proven; live pairing + populated-decoder verification deferred to a headless-`synocredential` follow-on (lead: `SYNO.Remote.Credential.Challenge/.Info`) or an operator-created relation. |

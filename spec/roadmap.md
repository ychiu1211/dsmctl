...
| [WI-090](work-items/WI-090-snapshot-replication-mutation.md) | P2 | `in_progress` | C | WI-089 | Guarded Snapshot Replication **relation create** source-profile → dest-profile for one share (temp_create → check_remote_conn → DR.Plan create v3 → poll), via hash-bound plan/apply. The destination admin password is resolved from its vault entry by dsmctl's audited resolver **at apply time only** — never in the plan/hash/logs/MCP args/model. Never-overwrite-dest + reachability guards, async-task postcondition; read-back upgrades the WI-089 replication decoder to live-verified. Failover/switchover/reprotect family stays read-only `can_*` reporting; sync/edit deferred. Live-verified nas51→nas255. |

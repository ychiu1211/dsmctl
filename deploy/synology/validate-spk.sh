...
for required in image.tar.gz image-metadata.json project/compose.yaml bin/dsmctl-synology-auth ui/config; do
...
grep -q 'platform-assertion-key-file=/run/secrets/platform.key' "$work/package/project/compose.yaml"

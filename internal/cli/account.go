...
func newAccountCommand(opts *options) *cobra.Command {
	command := &cobra.Command{Use: "account", Short: "Inspect and manage DSM local users and groups"}
	command.AddCommand(
...
func newAccountInventoryCommand(opts *options) *cobra.Command {
	...
	command := &cobra.Command{
		Use:   "inventory",
		Short: "Show local DSM users and groups",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			service, err := loadService(opts.configPath, terminalOTPProvider(cmd))
			if err != nil {
			...
			defer closeService(service)
			result, err := service.GetIdentityState(cmd.Context(), opts.nas)
			if err != nil {
				return err
			}
			if jsonOutput {

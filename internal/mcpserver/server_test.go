...
func TestNewRegistersToolSchemas(t *testing.T) {
	...
	var downloadSettingsPlanTool *mcp.Tool
	for _, tool := range tools.Tools {
		if tool.Name == "plan_download_station_settings_change" {
			downloadSettingsPlanTool = tool
		}
		if scope, ok := ToolScope(tool.Name); !ok || scope == "" {
			t.Errorf("MCP tool %q has no enforceable remote authorization class", tool.Name)
		}
	}
	if downloadSettingsPlanTool == nil {
		t.Fatal("plan_download_station_settings_change was not registered")
	}
	encodedOutputSchema, err := json.Marshal(downloadSettingsPlanTool.OutputSchema)
	if err != nil {
		t.Fatalf("marshal Download Station settings output schema: %v", err)
	}
	var outputSchema any
	if err := json.Unmarshal(encodedOutputSchema, &outputSchema); err != nil {
		t.Fatalf("decode Download Station settings output schema: %v", err)
	}
	observedSchema := findSchemaProperty(outputSchema, "observed")
	if observedSchema == nil || observedSchema["type"] != "object" {
		t.Fatalf("Download Station settings plan observed schema = %#v, want object", observedSchema)
	}
...
func findSchemaProperty(value any, name string) map[string]any {
	...
	case map[string]any:
		...
		for _, child := range typed {
			if found := findSchemaProperty(child, name); found != nil {
				return found
			}
		}
	case []any:
